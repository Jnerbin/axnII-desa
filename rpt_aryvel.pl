...
$K_radios = 0;
@Radios;
$dia_de_hoy = f6tof8($axn_fecha);
if ($DBI::errstr) {
   print ("$DBI::errstr<br>");
} else {

   my $xptrr = $dbh->prepare("SELECT distinct radio from Localizaciones order by radio ASC");
   $xptrr->execute();
   $K_radios = $xptrr->rows;
   @Radios = $xptrr->fetchrow_array;
   &ArmoListaIpUsr();
...
sub RptArranqueParada {
    ...
    my $sqlq_ve;
    if ($vehiculo eq "0") {
       $sqlq_ve = "SELECT * from Vehiculos where nro_ip in ($ip_string) order by descripcion";
...
#         while ($posicion[5] > 0 && ($Marcas > $XMarca) && ($parado < $min_stop)) {
         while ($posicion[5] > 0 && ($Marcas > $XMarca)) {
            $m_dist += distP1P2($pos_ant[3], $pos_ant[4], $posicion[3], $posicion[4]);
...
#            print "<th>$x_utmn</th>";
      ...
      if ( $fecha != $dia_de_hoy ) {
        my $h_24 = '23:59:59';
        ($str_stpo1, $m_tpo_mov1) = tpo_segs($m_hfin, $h_24);
        print "<th><font size='-1'>$str_stpo1</th>";
        $tot_stop += $m_tpo_mov1
      } else {
        my ($hhmm,$ddmmaa, $h_24) = FechaHora();
        ($str_stpo1, $m_tpo_mov1) = tpo_segs($m_hfin, $h_24);
        print "<th><font size='-1'>$str_stpo1</th>";
        $tot_stop += $m_tpo_mov1
      }
...
sub get_localizacion {
  ...
  my @poblaciones;
  my $ind = 0;
  my $sqlq =  "Select  SQRT( POW((utme - ?),2)+ POW((utmn - ?),2) ) as dist , nombre, radio, ";
  $sqlq .= "tipo_localizacion FROM Localizaciones  where (SQRT( POW((utme - ?),2)+ POW((utmn - ?),2) ) < radio) order by dist";
  my $ptr_loc = $dbh->prepare($sqlq);
  $ptr_loc->execute($x_utme, $x_utmn, $x_utme, $x_utmn);
  my ($mdist, $lnom, $lradio, $tloc) = $ptr_loc->fetchrow_array();
  $lradio = $lradio * 1;
#  print "$mdist $lradio $lnom $x_utme $x_utmn = ";
  if ( int($mdist) <= $lradio and $ptr_loc->rows > 0) {
    $x_ciudad = $lnom
  } else {
