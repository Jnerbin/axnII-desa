...
use vars qw ($dbh $cgi $Kmapa $Knivel $Kcateg $Klat1 $Klon1 $Klat2 $Klon2
             ...
             @regmap $MAPA_mts $KUTME $KUTMN $KrllCtrl);
...
#print "boton = $boton<br>";
   ...
   if (( $cgi->param('xymapa.x') && ($boton eq 'Acercar' || $boton eq 'Actualizar' || 
	$boton eq 'Alejar' || $boton eq 'Marcar') ) || $boton eq 'Actualizar' ){
      ...
      $KrllCtrl = $cgi->param('op_controlar');
      if ($Knivel == 1) {
          $DeltaXA = 0;
          $DeltaYA = 0;
      }
      ...
      if ($boton eq 'Marcar') {
         my $ins = $dbh->prepare("INSERT into RutasLatLon
                   (ruta, lat, lon, utme, utmn, controlar, radio)
                   VALUES (?,?,?,?,?,?)");
	 my $tpo_ctrl = 1;  
	 if ($KrllCtrl eq 'Ruta') {
	    $tpo_ctrl = 0;  
	 }	 
         $ins->execute($KRuta, $Klat1, $Klon1, $est, $nor,$tpo_ctrl);
...
sub MarcoPosicionesEnMapa {
  ...
  while (my @xr = $rut->fetchrow_array) {
    ...
    $imagen->Draw(fill=>$color, primitive=>'circle', points=>$xx.",".$yy." ".($xx+4).",".($yy+4));
    $imagen->Annotate(stroke=>'black', strokewidth=>5, text=>$xr[1], x=>($xx-6), y=>($yy-7));
    $imagen->Annotate(fill=>'white',  text=>$xr[1], x=>($xx-6), y=>($yy-7));
...
#  print "$latM, $lonM";
  ...
  $imagen->Draw(stroke=>'red', fill=>'red', primitive=>'line',
                strokewidth=> 4, antialias=> 'true', points=>$kkstr);
