...
@tbl_campos = (
   {name => 'nro_vehiculo', label => 'Cod.', sizesk => 3, sizefr => 3, req => 1, tipo => 'num'},
   ...
   {name => 'estado',       label => 'Estado',         sizesk => 1, sizefr => 1, req => 0, tipo => 'txt'},
   {name => 'nro_patente',  label => 'Nro Patente',    sizesk => 10,sizefr => 10,req => 1, tipo => 'txt'},
   {name => 'trayecto',     label => 'Trayecto',       sizesk => 4,sizefr => 10,req => 0, tipo => 'txt'},
   {name => 'tipo_marca',   label => 'TipoMarca',      sizesk => 1,sizefr => 1,req => 0, tipo => 'txt'},
);
...
#       if ( $user eq "invitado") {
#         print "No se Puede Dar de Baja como invitado<br>";
#       } else {	   
#          BajaItems();
#       } 
   }
   ...
   print start_form();
      ...
      foreach (0..$#tbl_campos) {
         if ($_ == 0 ) {
            ...
            print "<TR>";
            if ($_ == 5) {
               print "<td>$tbl_campos[$_]->{label}</td><td></td><td> ";
               print popup_menu(-name=>'estado', -values=>\@pclave, -labels=>\%phash, -default=>$datos2[$_]);
               print "</td>";
            } elsif ($_ == 7) {
               print "<td>$tbl_campos[$_]->{label}</td><td></td><td> ";
               print popup_menu(-name=>'trayecto', -values=>\@tclave, -labels=>\%thash, -default=>$datos2[$_]);
               print "</td>";
            } elsif ($_ == 2) {
                 print "<td align='left'>$tbl_campos[$_]->{label}</td><td></td>";
...
#                 print "<td><input name='$tbl_campos[$_]->{name}' size='$tbl_campos[$_]->{sizefr}' value='$disp'></td>";
                 print hidden(-name=>$tbl_campos[$_]->{name}, -default=>$disp);
            } else {
               print "<td align='left'>$tbl_campos[$_]->{label}</td><td></td>";
               if ($operacion eq "Modifico") {
                 my $disp = tipo_datos($datos2[$_], $tbl_campos[$_]->{tipo});
...
sub AltaItem {
   ...
   if ( $reg->rows == 0 ) {
      ...
      foreach (0..($#tbl_campos-1)) {
         ...
         if ($_ == 0 and $tbl_campos[$_]->{tipo} eq 'txt') {
            $vcampo = uc($vcampo);
         } elsif ( $tbl_campos[$_]->{tipo} eq 'txt') {
