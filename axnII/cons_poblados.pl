...
use vars qw ($dbh $cgi $Kmapa $Knivel $Kcateg $Klat1 $Klon1 $Klat2 $Klon2 $K_Estado
             ...
             $cb_fechor $CropX $CropY $XYout @dat_vehiculo @Ciudades $KFecha $KHora $KUPT
             $HTML_GM $URL_XML $URL_HTML $GM_ZOOM $GM_KEY $KCantV $KBorde $KimgLat1 $KimgLon1
             $KimgLat2 $KimgLon2 $K_Pueblos);
...
sub ArmarListaUltimaPosicion {
  ...
  if ($KUPT ne "OK") {
    if ($K_Estado eq 'Rodando') {
      ...
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion order by nro_ip");
      $ptrup->execute();
    }
  } else {
    if ($K_Estado eq 'Rodando') {
  ...
  while (my @ultimapos=$ptrup->fetchrow_array ) {
     ...
     if ($ptrlv >= 1) {
        ...
        my $l_hrstop = $xxhora;
        push @{$dat_vehiculo[$ind]}, $l_nom, $l_fec, $l_hor, $l_vel, $l_hors, $l_img, $l_hrstop;
        $ind += 1;
     } 
  }
}
...
sub DesplegarResultados {
  ...
  print "<TABLE BORDER='0' style='text-aLIgn: left; margin-left: auto; margin-right: auto;'><span style='font-family:Arial'>";
   print "<TR>";
    print "<td style='vertical-align: center; text-align: left;'><font size='1'><span style='font-family:Arial'>";
      my @xbot;
      if ($Knivel == 1) {
        if ($Kcateg == 1) {
          @xbot = ['Acercar'];
     ...
     print " $tb$tb$tb$tb ";
     print checkbox(-name=>'solohoy', -checked=>1, -value=>'OK', -label=>'Solo Marcas de Hoy');
     print " $tb$tb$tb$tb";
     print popup_menu(-name=>'stoprun', -values=>['Todos','Rodando','Parados'], -default=>$K_Estado);
     print " $tb$tb$tb$tb Marcar ";
     print popup_menu(-name=>'pueblos', -values=>['Nada','Capitales','Ciudades','Pueblos','Poblados','Todos'], -default=>$K_Pueblos);
     print " $tb$tb$tb$tb ";
     print  submit(-name=>'opcion', -value=>'Actualizar');
     print " $tb$tb$tb$tb Ver En ";
...
#         }
     ...
     &getPoblaciones($KimgLat1, $KimgLon1, $KimgLat2, $KimgLon2, 100);
     my $i = 0;
     for $i (0..$#Ciudades) {
       my ($px, $py) = latlon2xy($Ciudades[$i][0], $Ciudades[$i][1], @mapa);   
       $px = $px - $dtx - 1; 
       $py = $py - $dty - 1; 
       $imagen->Draw(fill=>"yellow", primitive=>'circle', points=>$px.",".$py." ".($px+2).",".($py + 2));
       if ($Knivel == 2) {
          $imagen->Annotate(fill=>'white', text=>$Ciudades[$i][2], x=>$px, y=>$py);
       }
     }
     my $tiempo=time();
...
#print "Klat1 = $Klat1 Klon1 + $Klon1<br>";
   ...
   if ($Knivel == 1 ) { 	# El mapa va ENTERO
      ($KimgLat1, $KimgLon1, $KimgLat2, $KimgLon2) = llcuadro(0,0,$regmap[2],$regmap[3]);
      if ($res  > 0 ) {
...
$KBorde = 4;
      ...
      ($KimgLat1, $KimgLon1, $KimgLat2, $KimgLon2) = llcuadro(($xx-$x_out/2), ($yy-$y_out/2), ($xx+$x_out/2), ($yy+$y_out/ 2));
...
sub llcuadro {
  my ($x1, $y1, $x2, $y2) = @_;
  my ($lat1, $lon1) = xy2latlon($x1, $y1, $Kmapa);
  my ($lat2, $lon2) = xy2latlon($x2, $y2, $Kmapa);
  return ($lat1, $lon1, $lat2, $lon2);
}

sub getPoblaciones {
  my ($la1, $lo1, $la2, $lo2, $poblacion) = @_;
#  print "($la1, $lo1, $la2, $lo2 $Kmapa) <br>" ;
  my $ptrp = $dbh->prepare("Select * from Poblaciones where poblacion > ? and tipo <> 'Barrio' and
                            (latitud < ? and latitud > ? and longitud > ? and longitud < ?)");
  $ptrp->execute($poblacion, $la1, $la2, $lo1, $lo2);
  my $ind = 0;
  while ( my @pob = $ptrp->fetchrow_array() ) {
    push @{$Ciudades[$ind++]}, $pob[1], $pob[2], $pob[5], $pob[6];
#    print " <br> $pob[5]";
  }
}
