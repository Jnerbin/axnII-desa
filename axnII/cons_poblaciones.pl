...
#print "MODIFICADO ($Klocnom, $Klocrad, $Kcodloc)<br>";
           my $upd=$dbh->prepare("UPDATE Poblaciones SET nombre = ?, radio = ? WHERE clave = ?");
           $upd->execute($Klocnom, $Klocrad, $Kcodloc);
        } elsif ($cgi->param('opcion') eq 'Borrar' ) { # Borrar el Punto
           my $tpl=$dbh->prepare("DELETE FROM Poblaciones WHERE clave = ?");
           $tpl->execute($Kcodloc);
...
print "clave = $Kcodloc<br>";
      my $ptrloc=$dbh->prepare("SELECT * FROM Poblaciones WHERE clave = ?");
...
sub DesplegarResultados {
  ...
  my $ptrloc=$dbh->prepare("SELECT clave, latitud, longitud, nombre, radio, depto
			    FROM Poblaciones ORDER by nombre");
  $ptrloc->execute();
  ...
  for (0..$#{$arr_loc}) {
      ...
      $lhash{$lclave[$kk]}= $arr_loc->[$_][3]." (".$arr_loc->[$_][5].")";
  }
  print start_form();
