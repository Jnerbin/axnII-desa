...
#my $quiensoy 			= $cgi->cookie('TRACK_USER_ID');
#($user, $pass, $nom_base) 	= split(/=/,$quiensoy);
#my $base			= "dbi:mysql:".$nom_base;
#$base = $base . ";host=192.168.0.222;port:3306";
...
#print "<br>Lat = $PAR_LAT Lon = $PAR_LON<br>";
   AnalizoOpciones();
   print $cgi->end_html();
   $dbh->disconnect;
}
...
sub AnalizoOpciones {
   ...
   my $x_grabar      = $cgi->param('opcion');
     print "$x_mapa<br>";
#   if ($x_grabar ne "Grabar") {
   if ($x_mapa eq "") {
     $x_mapa = "Montevideo";
     VariablesMapa($x_mapa);
     my ($x_latitud, $x_longitud) = split(/,/, $PAR_LAT);

#     my $x_latitud = $PAR_LAT;
#     my $x_longitud = $PAR_LON;
#     my $Glat = $PAR_LAT;
#     my $Glon = $PAR_LON;
#     print ("---> $x_latitud  $x_longitud <br>");

     &MarcoLatLon($x_latitud, $x_longitud);
   } else {
     print "Alta del Item<br>";
...
#====================================================================================
sub DondeEstanTodos {
   my $sqlq = "SELECT * FROM Poblaciones";
   ...
   if ($up_cant > 0 ) {
      ...
      $marca->Read("/var/www/html/axnII/iconos/pto_rojo.png");
      my $xtexto = "";
      my $strpunto;
      my $addx = 0;
      my $addy = 0;
#      $row = $ptr->fetchall_arrayref;
      while (@row = $ptr->fetchrow_array) {
         my ($xo, $yo, $encuadra) = ll2xy($row[1], $row[2],0);
         $addx  = $xo - 8;
         $addy  = $yo - 8;
$imagen->Composite(image=>$marca, compose=>'Over', x=>$addx, y=>$addy);
print "HOLA<br>";
##         $strpunto  = $xo.",".$yo." ". $addx.",".$addy;
#         print "1 - $row[5]<br>";
#         $imagen->Draw(fill=>'red', primitive=>'circle', points=>$strpunto);
##         $imagen->Draw(fill=>"blue", primitive=>'circle', points=>$xo.",".$yo." ". ($xo + 4) .",". ($yo + 4) );
      }
...
#         $xx->Read($axn_path_map."/toro.png");
         $xx->Read("/var/www/html/axnII/iconos/pto_rojo.png");
         $imagen->Composite(image=>$xx, compose=>'Over', x=>$xc, y=>$yc); 
#$imagen->Draw(fill=>"red", primitive=>'circle', points=>$mitadx.",".$mitady." ". ($mitadx + 4) .",". ($mitady + 4) );
...
#print "GLAT = $Glat GLON = $Glon X_SI=$Xsi Y_SI=$Ysi<br>";
   ...
   print "<td>";
     print "<TABLE>";
       print "<TR><td></td><td><font size='5'>Registro de Posicion</td></TR>";
       print "<TR><td></td></TR>";
       print "<TR><td></td></TR>";
       print "<TR><td></td></TR>";
       print "<TR><td></td></TR>";
       print "<TR>";
          print "<td align='left'><font size='-1'>Descripcion</td>";
          print "<td><font size='-1'><input name='nombre' value = '$Xnombre' size='40'></td>"; 
       print "</TR>";
       print "<TR>";
          print "<td align='left'><font size='-1'>Tipo Localizacion</td><td>";
