...
use vars qw ($dbh $cgi $Kmapa $Knivel $Kcateg $Klat1 $Klon1 $Klat2 $Klon2
             ...
             $tb $imagen $KUrlImagen $DeltaXA $DeltaYA $Klocnom $Kcodloc 
   ...
   if ( $cgi->param('xymapa.x') && 
      ...
      if ($Klocnom ne "") {
        if ($cgi->param('opcion') eq 'Modificar') { # Modificamos Descripcion
           my $upd=$dbh->prepare("UPDATE Localizaciones SET nombre = ? WHERE codigo = ?");
           $upd->execute($Klocnom, $Kcodloc);
           ...
           my $ins = $dbh->prepare("INSERT into Localizaciones
                     ...
                     VALUES (?,?,?,?,?,?,?)");
           $ins->execute($Klat1, $Klon1, $est, $nor, $Klocnom, 20, $tpl);
...
sub MarcoPosicionesEnMapa {
  ...
  if ( $opcion eq "Marcar Todas") {
    my $ptr = $dbh->prepare("SELECT * from Localizaciones WHERE tipo_localizacion = ?");
    my $mtl = $cgi->param('gr_tipol');
    $KTipo = $mtl;
    $ptr->execute($KTipo);
    while ( my @loc = $ptr->fetchrow_array ) {
       my ($px, $py) = latlon2xy($loc[1], $loc[2], @mapa);
       $px = $px - $dtx;
       $py = $py - $dty;
        if ($px < $CropX && $py < $CropY && $Knivel > 1) {
           ...
           $lddx += 5;
           $imagen->Annotate(stroke=>'blue', strokewidth=>5, text=>$loc[5], x=>$lddx, y=>$lddy);
           $imagen->Annotate(stroke=>'white', strokewidth=>1, text=>$loc[5], x=>$lddx, y=>$lddy);
	}
    }
  } else { 
    my $xc = $CropX / 2 - 8;
    ...
    print "<td  style='vertical-align: top;'>";
      print "<TABLE BORDER='0' style='text-align: left; margin-left: auto; margin-right: auto;'>";
       ...
       print "<TR>";
        ...
        print popup_menu(-name=>'gr_tipol', -values=>\@tclave, -default=>$KTipo, -labels=>\%thash, -override=>'1');
        print submit(-name=>'opcion', -value=>'Marcar Todas');
        print "</td>";
       print "</TR>";
       print "<TR>";
        print "<td><font size='-1'>Descripcion</td>";
        print "<td><input name='x_descripcion' value = '$Klocnom' size = '50'>";
        print "</td>";
       print "</TR>";
       print "<TR>";
