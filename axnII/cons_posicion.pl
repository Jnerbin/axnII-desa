...
use vars qw ($dbh $cgi $Kmapa $Knivel $Kcateg $Klat1 $Klon1 $Klat2 $Klon2 $K_Estado
             ...
             $HTML_GM $URL_XML $URL_HTML $GM_ZOOM $APIS_KEY $KCantV $KBorde $KCantVehiculos
             @ip_user $ip_string);
...
$APIS_KEY = "ABQIAAAAZNxuIilzeBkODlv1C8plNRQlVCi5kAX9Yhz9eUJWgNXBF2F00BR5hCwwFt7oYvsW1TLJaUeUlyBgzw";
...
if ($DBI::errstr) {
   ...
   $Klon1  = -10;
   $Klon2  = -100;
#   print $cgi->start_html("DTSA Track");
&ArmoListaIpUsr();
   &html_cab();
...
sub ArmarListaUltimaPosicion {
  ...
  if ($KUPT ne "OK") {
    if ($K_Estado eq 'Rodando') {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad > ? order by nro_ip");
      $ptrup->execute(0);
    } elsif ($K_Estado eq 'Parados') {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad = ? order by nro_ip");
      $ptrup->execute(0);
    } else {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) order by nro_ip");
      $ptrup->execute();
    }
  } else {
    if ($K_Estado eq 'Rodando') {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad > ? and fecha = ? order by nro_ip");
      $ptrup->execute(0,$KFecha);
    } elsif ($K_Estado eq 'Parados') {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad = ? and fecha = ? order by nro_ip");
      $ptrup->execute(0,$KFecha);
    } else {
      $ptrup = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and fecha = ? order by nro_ip");
      $ptrup->execute($KFecha);
    }
  }
  $KCantVehiculos = $ptrup->rows;
#print "Son : $KCantVehiculos<br>";
  my $ind = 0;
  while (my @ultimapos=$ptrup->fetchrow_array ) {
     my $posxx = $dbh->prepare("SELECT Max(hora) FROM Posiciones WHERE nro_ip = ? and fecha = ? and velocidad > ?");
     $posxx->execute($ultimapos[0], $ultimapos[1], 0);
     my $xxhora = $ultimapos[2];
     if ($posxx->rows > 0) {
         my $xxhora = $posxx->fetchrow_array();
     }
     ...
     if ($ptrlv >= 1) {
        ...
        my $ptrp = $dbh->prepare("Select ult_hora_sistema, motor_onoff from UltimaPosicion where nro_ip = ?");
        $ptrp->execute($ultimapos[0]);
        my ($xhs, $onoff) = $ptrp->fetchrow_array; # Ultima hora andando
        push @{$dat_vehiculo[$ind]}, $l_nom, $l_fec, $l_hor, $l_vel, $l_hors, $l_img, $xhs, $vehiculos[2], $onoff;
        $ind += 1;
     } 
  }
}
...
sub html_cab {
    ...
    print "<head><title>DTSA Track</title>";
...
print '  else';
print '  href=mylink.href;';
print '  window.open(href, "DTSA", "status=no, width=700,height=500,resizable=1");';
print '  return false;';
print '  }';
print '  //-->';
print '  </SCRIPT>';
...
#      print "$XYout $CX $CY<br>";
  ...
  if ($KUPT ne "OK") {
    if ($K_Estado eq 'Rodando') {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad > ? order by nro_ip");
      $ptr->execute(0);
    } elsif ($K_Estado eq 'Parados') {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad = ? order by nro_ip");
      $ptr->execute(0);
    } else {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) order by nro_ip");
      $ptr->execute();
    }
  } else {
    if ($K_Estado eq 'Rodando') {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad > ? and fecha = ? order by nro_ip");
      $ptr->execute( $KFecha);
    } elsif ($K_Estado eq 'Parados') {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and velocidad = ? and fecha = ? order by nro_ip");
      $ptr->execute( $KFecha);
    } else {
      $ptr = $dbh->prepare("SELECT * FROM UltimaPosicion where nro_ip in ($ip_string) and fecha = ? order by nro_ip");
      $ptr->execute( $KFecha);
    }
  }
      $cant = $ptr->rows;
...
sub DesplegarResultados {
  my (@datos) = @_;
@datos = sort {$a->[1] cmp $b->[1] || $a->[0] cmp $b->[0]} @datos;
  my ($i, $j);
  ...
  print "<TABLE BORDER='0' style='text-aLIgn: left; margin-left: auto; margin-right: auto;'><span style='font-family:Arial'>";
   print "<TR>";
    print "<td style='vertical-align: center; text-align: left;'><font size='2'><span style='font-family:Arial'>";
     ...
     print " $tb$tb$tb$tb";
     print checkbox(-name=>'solohoy', -checked=>0, -value=>'OK', -label=>'Solo Marcas del Dia de Hoy');
     print " $tb$tb$tb$tb";
  ...
  print "<TABLE BORDER='0' style='text-aLIgn: left; margin-left: auto; margin-right: auto;'><span style='font-family:Arial'>";
   print "<TR align='center'><td>";
    ...
    print "<td  style='vertical-align: top;'>";
      ...
      print "</TABLE>";
      if ($KUPT eq 'KK') {
         print "<TABLE BORDER='1' cellspacing=1 cellpadding=2 id=header>";
         print "<COL WIDTH=100><COL WIDTH=66><COL WIDTH=47><COL WIDTH=37><COL WIDTH=65><COL WIDTH=38>";
      } else {
         print "<TABLE BORDER='1' cellspacing=1 cellpadding=2 id=header>";
         print "<COL WIDTH=100><COL WIDTH=47><COL WIDTH=37><COL WIDTH=65><COL WIDTH=38>";
      }
      ...
      if ($KUPT eq 'KK') {
        print "<DIV STYLE='overflow: auto; width: 380; height: 400; padding:0px; margin: 0px;'>";
         print "<TABLE BORDER='1'>";
         print "<COL WIDTH=80><COL WIDTH=60><COL WIDTH=40><COL WIDTH=30><COL WIDTH=60><COL WIDTH=40>";
      } else {
         print "<DIV STYLE='overflow: auto; width: 320; height: 400; padding:0px; margin: 0px;'>";
         print "<TABLE BORDER='1'>";
         print "<COL WIDTH=80><COL WIDTH=40><COL WIDTH=30><COL WIDTH=60><COL WIDTH=40>";
      }
      ...
      for $i (0..$#datos) {
        ...
        $vup = $datos[$i][2];
        my $vtime2 = 0;
        if ( $datos[$i][3] == 0 ) { # La velocidad es cero...
          $vtime2 = POSIX::mktime( "00", substr($xvup,3,2), substr($xvup,0,2), 
                      substr($vfe,0,2), substr($vfe,3,2)-1, substr($vfe,6,2)+100);
        } else {
          $vtime2 = POSIX::mktime( "00", substr($vup,3,2), substr($vup,0,2), 
                      substr($vfe,0,2), substr($vfe,3,2)-1, substr($vfe,6,2)+100);
        }
        my $ahora=time();
#        my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday) = localtime($ahora);
#        my $ts2 = POSIX::mktime($sec,$min,$hour,$mday,$mon,$year);
        my $tpdif = abs($vtime2 - ($ahora) );
#print "$tpdif<br>";
        my $xxvel=$datos[$i][3];
        if ( $xxvel == 0 ) {
        ...
        if ($datos[$i][8] < 1 ) { $bkgc = "black"; }
#print "$ahora $vtime2 $vtime $tpdif <br>";
        print "<TR>";
        for $j (0..($#{ $datos[$i] } - 1 )) {
           if ( $j == 0 ) {
             print "<td width=100 style='background-color: $bkgc;text-align'>";
             my $bt = substr($datos[$i][0],0,10);
             ...
             if ($KUPT eq 'KK') {
               ...
               if ( $xf <  $xH ) { 
                  $bkgc = "red";
                  print "<td width=60 style='background-color: $bkgc;text-align: center;'>";
               } else {
                  print "<td width=60 >";
               }
               print $datos[$i][$j];
...
#           if ($j == 3 && $tpdif > 60 && $datos[$i][3] == 0) {
           if ($j == 3 && $tpdif > 0 && $datos[$i][3] == 0) {
              if ($K_Estado eq 'Parados' or $K_Estado eq 'Todos') {
#                my $ptrp = $dbh->prepare("Select ult_hora_sistema from UltimaPosicion where nro_ip = ?");
#                $ptrp->execute($datos[$i][7]);
#                my $xhs = $ptrp->fetchrow_array; # Ultima hora andando
                print "<td width=60 >";
                my $str_stop = seg2dias($tpdif);
                print $str_stop;
                print " </td>";
                print "<td width=40 >";
                print substr($xvup,0,5)."</td>";
              }
           }
        }
        print "</TR>";
  ...
  print "</TABLE>";
  print "<TABLE>";
   print "<TR><td style='background-color: black; color:white;font-size:70%'>Detenido. Motor OFF Apagado";
   print "<td style='background-color: red;font-size:70%'>STOP. Mas de 30 min. Motor ON";
   print "<td style='background-color: orange;font-size:70%'>STOP. Menos de 30 min. Motor ON";
   print "<td style='background-color: yellow;font-size:70%'>STOP. Menos de 10 min. Motor ON";
   print "<td style='background-color: limegreen;font-size:70%'>Rodando o Detenido Menos de 1 min.";
   print "<td style='background-color: magenta; color:black;font-size:70%'>Sin Senal Mas de 30 min.";
   print "</TR>";
  print "</TABLE>";
...
sub seg2dias {
  my ($xseg) = @_;
  my $xdias = int($xseg / 86400);
  my $xhora = int(($xseg - ($xdias * 86400)) / 3600);
  my $xmins = int ( ($xseg - ($xhora * 3600) - ($xdias * 86400)) / 60);
  $xseg =  $xseg - ($xmins * 60 ) - ($xhora * 3600) - ($xdias * 86400);
  if ( $xdias > 0) {
#    return "[$xdias d] $xhora hr $xmins\'";
    $xhora += ( 24 * $xdias );
    return "$xhora hr";
  } elsif ( $xhora > 0 ) {
    return "$xhora hr $xmins\'";
  } elsif ( $xmins > 0) {
    return "$xmins\' $xseg\'\'";
  } else {
    return "$xseg\'\'";
...
#         }
     ...
     $imagen->Annotate(fill=>'blue', pointsize=>18, text=>$Kmapa, gravity=>'North');
     $imagen->Set(quality=>50);
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    <title>DTSA Track</title>
    <script src=\"http://maps.google.com/maps?file=api&v=2&key=$APIS_KEY\" type=\"text/javascript\">
    </script>
    <SCRIPT TYPE="text/javascript">
    <!--
    window.focus();
  ...
  <body onload="onLoad()" onunload="GUnload()">
    ...
    <table border=1 style="width: 100%; height: 100%">
      ...
      <tr>
        <td height = 25px>
           Rango Vel:
             <img src="/axnII/iconos/dir/stop.png" align="middle">1</img>
             <img src="/axnII/iconos/dir/blue-000.png" align="middle">40</img>
             <img src="/axnII/iconos/dir/cyan-000.png" align="middle"> 60 </img>
             <img src="/axnII/iconos/dir/green-000.png" align="middle"> 80 </img>
             <img src="/axnII/iconos/dir/yellow-000.png" align="middle"> 90 </img>
             <img src="/axnII/iconos/dir/magenta-000.png" align="middle"> 100</img>
             <img src="/axnII/iconos/dir/red-000.png" align="middle">+</img>
     ...
     xIcon.shadow     = "http://www.controlflotacom.do/axnII/iconos/dir/shadow.png";
     xIcon.iconSize   = new google.maps.Size(24, 24);
     xIcon.shadowSize = new google.maps.Size(12, 12);
     xIcon.iconAnchor = new google.maps.Point(12, 12);
     xIcon.infoWindowAnchor = new google.maps.Point(12, 4);
     var map = new google.maps.Map(document.getElementById("map"));
     map.addControl(new google.maps.LargeMapControl());
     map.addControl(new google.maps.MapTypeControl());
//     map.setCenter(new google.maps.LatLng( $latm, $lonm), $GM_ZOOM);
     ...
     map.setMapType( google.maps.HYBRID_MAP );
      ...
      function createMarker(point, texto, velx, nombre, direccion, m_id) {
        var marker = new google.maps.Marker(point,get_icon(velx,direccion));
...
if ( $KCantVehiculos == 1 ) {
     ...
     $sqlq = "SELECT * from Mapas2 WHERE
              ( (lat1 >= ? AND lat2 <= ?) AND
                (lon1 <= ? AND lon2 >= ?) AND categoria > 1 )
              ORDER BY categoria ASC, nivel ASC";
}
     $ptr = $dbh->prepare($sqlq);
