...
#    print ("$sqlq_ve<br>");
    ...
    if ($cant_v < 1) {
       ...
       my $WorkB = Spreadsheet::WriteExcel->new("/axnII/tmp/XXX.xls");
       while (my @datos_v = $ptr_v->fetchrow_array ) {
          print $cgi->h3("Vehiculo: $datos_v[1]");
          &ListoVehiculoAP($datos_v[2], $fecha, $hrini, $hrfin, $min_stop, $datos_v[8], $WorkB); 
       }
    }
}
...
#====================================================================================
sub ListoVehiculoAP {
   my ($ip, $fecha, $hrini, $hrfin, $min_stop, $TipoMarca, $WB) = @_;
   my $Planilla = $WorkB->add_worksheet();
...
sub get_localizacion {
    ...
    my @poblaciones;
    my $ind = 0;
    while ( $ind <= $K_radios and  $cuantos == 0) {
       my $XRadio = $Radios[$ind];
