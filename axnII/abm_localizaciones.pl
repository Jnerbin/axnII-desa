...
#   if ( $cgi->param('xymapa.x') && $cgi->param('zm_mas') ) {
   if ( $cgi->param('xymapa.x') && 
      ...
      if ($Klocnom ne "") {
        ...
        } elsif ($cgi->param('opcion') eq 'Borrar' ) { # Borrar el Punto
           my $tpl=$dbh->prepare("SELECT * FROM Localizaciones WHERE codigo = ?");
           $tpl->execute($Kcodloc);
           my @arraux=$tpl->fetchrow_array();
           $KTipo = $arraux[7]; 
           my $upd=$dbh->prepare("DELETE From Localizaciones WHERE codigo = ?");
           $upd->execute($Kcodloc);
           $tpl=$dbh->prepare("SELECT * FROM Localizaciones WHERE codigo = (Select max(codigo) from Localizaciones WHERE tipo_localizacion = ?)");
           $tpl->execute($KTipo);
...
sub DesplegarResultados {
  ...
  my $ptrloc=$dbh->prepare("SELECT codigo, nombre, tipo_localizacion 
			    FROM Localizaciones ORDER by tipo_localizacion, nombre");
  $ptrloc->execute();
  ...
  for (0..$#{$arr_loc}) {
      ...
      $lhash{$lclave[$kk]}= $arr_loc->[$_][2]."-".$arr_loc->[$_][1];
      if ($Kcodloc > 0) {
         $tp_loca = $arr_loc->[$_][2];
      }
  }
  ...
  print "<TABLE BORDER='0' style='text-align: left; margin-left: auto; margin-right: auto;'>";
   print "<TR align='center'>";
    ...
    print "<td  style='vertical-align: top;'>";
      print "<TABLE BORDER='0' style='text-align: left; margin-left: auto; margin-right: auto;'>";
       ...
       print "</TR>";
       print "<TR>";
        print "<td><font size='-1'>Descrip.</td>";
        print "<td><input name='x_descripcion' value = '$Klocnom' size = '30'>";
        print "</td>";
       print "</TR>";
       print "<TR>";
        print "<td><font size='-1'>Radio </td>";
        print "<td><font size='-1'><input name='x_radio' value = '$Klocrad' size = '6'> MTS de Afectacion";
        print "</td>";
       print "</TR>";
       print "<TR>";
        ...
        if ($Klocnom eq "") {
          ...
          print submit(-name=>'opcion', -value=>'Borrar');
        }
        print "</td>";
        print"<td>";
       print "</TR>";
   ...
   print "</TR>";
  print "</TABLE>";
  print "</td><td valign = 'top'><font size='-1'>";
          print"<TABLE border = '1' valign = 'top'>";
	  my $pll= $dbh->prepare('Select * from Localizaciones where tipo_localizacion = ? order by nombre');
          $pll->execute($KTipo);
          print "<tr><td>Nombre<td> <td>Radio<td></tr>";
          while (my @vtl = $pll->fetchrow_array()) {
             print "<tr><td>$vtl[5]<td> <td>$vtl[6]<td>";
#             print "<td>";
#             print submit(-name=>'opcion', -value=>'Ver');
#             print "<td>";
             print "</tr>";
          } 
          print"</TABLE>";
